<?php 
// session_start();
include '../models/EstadosDeCuenta.php';
$mdl_estadosDeCuenta = new EstadosDeCuenta();
...
switch ($_GET['op']) {
	case 'listarProyectos':
		$respuesta = $mdl_estadosDeCuenta->listarProyectos();
		$data = Array();
		while ($row = $respuesta->fetch_object()) {
			...
			$data[] = array(
				"0"=>$row->id,
				"1"=>'<a href="../estado-de-cuenta-usuario/?id_usuario='.$row->id.'" title="">'.$row->nombre." ".$row->apellido.'</a>',
				"2"=>$row->correo,
				"3"=>'$ '.number_format($row->valor_ganado),
				// "4"=>$avance,
	...
	case 'cargarSelectCliente':
			$respuesta = $mdl_estadosDeCuenta->listarActivos();
			while ($row = $respuesta->fetch_object()) {
				echo '<option value='.$row->id.'>'.$row->nombre.'</option>';
			}
		break;
	case 'eliminarProyecto':
		$eliminarUsuarioProyectoPorID = $mdl_estadosDeCuenta->eliminarUsuarioProyectoPorID($id_proyecto);
		if ($eliminarUsuarioProyectoPorID) {
			$eliminarProyectoPorID = $mdl_estadosDeCuenta->eliminarProyectoPorID($id_proyecto);	
